...
const GalpaoPage = () => {
  ...
  const handleSetorClick = (setorId: string) => {
    navigate(`/setores/${setorId}`);
  };

  const handleEditSetorClick = (setorId: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleNavigateTodosItens = () => {
    navigate(`/galpoes/${id}/itens`); // Navegar para a página de todos os itens do galpão
  };
      ...
      <Container component={motion.div} initial="hidden" animate="visible" variants={containerVariants} sx={{ textAlign: 'center' }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Typography
            ...
            gutterBottom
          >
            Veja todos os itens do galpão:
          </Typography>
          <Button
            variant="outlined"
            color="secondary"
            onClick={handleNavigateTodosItens} // Alterado para navegar para a página de itens
            sx={{ display: 'block', width: '100%', padding: 2, fontWeight: 'bold', fontSize: 16, border: '2px solid #1D78AC', '&:hover': {
              borderWidth: '3px', // Ensure the border thickness remains the same on hover
            }, }}
          >
            Itens
          </Button>
